            ...
            if (beg!=ImString::npos) {
                ...
                if (end!=ImString::npos) {
                    ...
                    if (originalLineNumber < numPrefixLines) printf("Error in autogenerated preprocessor definition code:\n%s", logString.c_str());
                        ...
                        int netLineNumber = (originalLineNumber - numPrefixLines);
                        ...
                        logString = logString.substr(0,beg);
                        logString+=&tmp[0];
                        if (errorIsInPrefixFile) logString+="i";
                        logString+=logString.substr(end);
...
static inline void ImImpl_MergeTwoSourceFiles(ImVector<char>& rv,const char* firstFile,const char* secondFile,int* pFirstFileNumLinesOut)    {
    ...
    rv.resize(lenff+lensf+1);strcpy(&rv[0],firstFile);strcat(&rv[0],secondFile);
    if (pFirstFileNumLinesOut) {
        *pFirstFileNumLinesOut = 1;
...
{
    IM_ASSERT(!optionalVertexShaderCodePrefix || !*optionalVertexShaderCodePrefix);         // TODO
    IM_ASSERT(!optionalFragmentShaderCodePrefix || !*optionalFragmentShaderCodePrefix);     // TODO
    ...
    if (mustProcessVertexShaderSource)  {
        ...
        ImVector<char> tmp;int sourcePrefixNumLines = 0;
        if (sourcePrefix) {
            ImImpl_MergeTwoSourceFiles(tmp,sourcePrefix,*vertexShaderSource,&sourcePrefixNumLines);
            const GLchar* pSource = &tmp[0];
            vertexShader = ImImpl_CreateShader(GL_VERTEX_SHADER,&pSource,pOptionalOptions,"VertexShader",sourcePrefixNumLines);
        }
    ...
    if (mustProcessFragmentShaderSource)    {
        ...
        ImVector<char> tmp;int sourcePrefixNumLines = 0;
        if (sourcePrefix) {
            ImImpl_MergeTwoSourceFiles(tmp,sourcePrefix,*fragmentShaderSource,&sourcePrefixNumLines);
            const GLchar* pSource = &tmp[0];
            fragmentShader = ImImpl_CreateShader(GL_FRAGMENT_SHADER,&pSource,pOptionalOptions,"FragmentShader",sourcePrefixNumLines);
        }
...
GLuint ImImpl_CompileShadersFromFile(const char* vertexShaderFilePath, const char* fragmentShaderFilePath, ImImpl_CompileShaderStruct* pOptionalOptions, bool allowProcessingASingleIncludeDirectivePlacedAtTheFirstLineOfTheShaderCode) {
    ...
    bool vok = ImGuiHelper::GetFileContent(vertexShaderFilePath,    vertexShaderBuffer,     true,"r",true);
    bool fok = ImGuiHelper::GetFileContent(fragmentShaderFilePath,  fragmentShaderBuffer,   true,"r",true);
    if (!vok || !fok)    {
    ...
    const char *pStartVertexShaderBuffer   = &vertexShaderBuffer[0];
    const char *pStartFragmentShaderBuffer = &fragmentShaderBuffer[0];
    ...
    if (allowProcessingASingleIncludeDirectivePlacedAtTheFirstLineOfTheShaderCode)  {
        ...
        if (includeNameVS.size()>0)   {
            ...
            if (vok) pStartPrefixVertexShaderBuffer = &prefixVertexShaderBuffer[0];
        }
        ...
        if (includeNameFS.size()>0)   {
            ...
            if (fullPathIncludeNameVS.size()>0 && strcmp(&fullPathIncludeNameVS[0],&fullPathIncludeNameFS[0])==0)   {fok=vok;pStartPrefixFragmentShaderBuffer=pStartPrefixVertexShaderBuffer;}
                ...
                if (fok) pStartPrefixFragmentShaderBuffer = &prefixFragmentShaderBuffer[0];
            }
        }
        if (!vok || !fok)    {
            printf("Error in ImImpl_CompileShadersFromFile(\"%s\",\"%s\"). ",vertexShaderFilePath,fragmentShaderFilePath);
