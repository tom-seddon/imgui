...
struct ImGuiTexList {
    ...
    struct DrawCmdData {
        const ImDrawCmd* cmd;
        int vtx_offset;
        DrawCmdData(const ImDrawCmd* _cmd,int _vtx_offset) : cmd(_cmd),vtx_offset(_vtx_offset) {}
    };
    ImVector<DrawCmdData> cmds;
    int vtx_offset;
    inline static int Compare(const void* o1,const void* o2) {
        return static_cast<const ImGuiTexList*>(o1)->texId < static_cast<const ImGuiTexList*>(o2)->texId ? -1 : 1;
    }
    inline static ImGuiTexList* Find(const ImVector<ImGuiTexList>& list,GLuint texId)    {
        for (size_t i=0,isz=list.size();i<isz;i++)  {
            const ImGuiTexList& l = list[i];
            if (l.texId==texId) return const_cast<ImGuiTexList*>(&l);
        }
        return NULL;
    }
};
...
const int cmd_lists_count = draw_data->CmdListsCount;
    ...
    for (int n = 0; n < cmd_lists_count; n++)   total_vtx_count += cmd_lists[n]->VtxBuffer.size();
...
#   define IMGUI_DOES_NOT_SORT_TEXTURES   // faster ? much more texture switches vs no preprocessing...

    // TODO: when !IMGUI_DOES_NOT_SORT_TEXTURES better sort lists before calling glBufferSubData
    ...
    //const GLuint imguiTexId = (GLuint)(intptr_t) ImGui::GetIO().Fonts->TexID;
#   ifdef IMGUI_DOES_NOT_SORT_TEXTURES
    GLuint texId = 0,lastTexId=0;
    int vtx_offset = 0;
    for (int n = 0; n < cmd_lists_count; n++)
    {
        const ImDrawList* cmd_list = cmd_lists[n];
        const ImDrawCmd* pcmd_end = cmd_list->CmdBuffer.end();
        //int textureChangeCnt = 0;
        for (const ImDrawCmd* pcmd = cmd_list->CmdBuffer.begin(); pcmd != pcmd_end; pcmd++)  {
            if (pcmd->UserCallback) pcmd->UserCallback(cmd_list, pcmd);
            else {
                texId = (GLuint)(intptr_t)pcmd->TextureId;
                if (texId!=lastTexId)   {
                    //fprintf(stderr,"%d-%d) texture change: %d -> %d\n",n,++textureChangeCnt,(int)lastTexId,(int)texId);
                    glBindTexture(GL_TEXTURE_2D, texId);
                    lastTexId = texId;
                }
...
#   else  // IMGUI_DOES_NOT_SORT_TEXTURES
    GLuint texId = 0;
    int vtx_offset = 0;
    for (int n = 0; n < cmd_lists_count; n++)
    {
        ...
        static ImVector<ImGuiTexList> texLists;texLists.clear();
        // Fill texLists:
        ImGuiTexList* pLastList = NULL;
        for (const ImDrawCmd* pcmd = cmd_list->commands.begin(); pcmd != pcmd_end; pcmd++)  {
            texId = (GLuint)(intptr_t)pcmd->texture_id;
            if (pLastList && texId==pLastList->texId)  {
                pLastList->cmds.push_back(ImGuiTexList::DrawCmdData(pcmd,vtx_offset));
                vtx_offset += pcmd->vtx_count;
                continue;
            }
            if ( (pLastList = ImGuiTexList::Find(texLists,texId))==NULL )   {
                const size_t sz = texLists.size();
                texLists.push_back(ImGuiTexList());
                pLastList=&texLists[sz];
                pLastList->texId=texId;
            }
            pLastList->cmds.push_back(ImGuiTexList::DrawCmdData(pcmd,vtx_offset));
            vtx_offset += pcmd->vtx_count;
        }
        // Draw texLists:
        const int texListsSize=(int)texLists.size();
        if (texListsSize>0) {
            //qsort(&texLists[0],texListsSize,sizeof(ImGuiTexList),&ImGuiTexList::Compare);   // Needed ? If images look dark, try this line [This line should be useless if every lists in each ImGui::Window starts with the default ImGui texture: hope it's true!]
            //for (int i=0;i<texListsSize;i++) fprintf(stderr,"[%d] texIds[%d]=%d (numComds:%d)\n",n,i,(int)texLists[i].texId,(int)texLists[i].cmds.size()); // Debug
            // Process sorted cmds:
            for (int i=0;i<texListsSize;i++)    {
                const ImGuiTexList& l = texLists[i];
                glBindTexture(GL_TEXTURE_2D, l.texId);
                for (size_t j=0,jsz=l.cmds.size();j<jsz;j++)  {
                    const ImGuiTexList::DrawCmdData& cd = l.cmds[j];
                    const ImDrawCmd* pcmd = cd.cmd;
                    if (pcmd->user_callback) pcmd->user_callback(cmd_list, pcmd);
                    else {
                        glScissor((int)pcmd->clip_rect.x, (int)(height - pcmd->clip_rect.w), (int)(pcmd->clip_rect.z - pcmd->clip_rect.x), (int)(pcmd->clip_rect.w - pcmd->clip_rect.y));
                        glDrawArrays(GL_TRIANGLES, cd.vtx_offset, pcmd->vtx_count);
                    }
                }
            }
        }
...
#   endif // IMGUI_DOES_NOT_SORT_TEXTURES
    ...
    for (int n = 0; n < cmd_lists_count; n++)
    {
        ...
        const GLuint imguiTexId = (GLuint)(intptr_t) ImGui::GetIO().Fonts->TexID;GLuint texId = 0,lastTexId=0;
        int vtx_offset = 0;
        const ImDrawCmd* pcmd_end = cmd_list->commands.end();
        for (const ImDrawCmd* pcmd = cmd_list->commands.begin(); pcmd != pcmd_end; pcmd++)
        {
